...
def getAverage():
  sql = '''SELECT pd.YEAR_OF_RETIREMENT,
            ROUND(AVG(pd.PENSION_VALUE) / AVG(pd.YEARS_OF_SERVICE), 2) AS AVERGE
  ...
  df = pd.DataFrame(average, 
                    columns=['Year_of_retirement', 
                              'Aveage_pension_PER_Years_of_service'])
